...
+ (NSString *)findMainBundleInFolder:(NSString *)folderPath
                    expectedFileName:(NSString *)expectedFileName
                               error:(NSError **)error
{
    NSString *result = [self findMainBundleInFolder:folderPath
                                 expectedFileName:expectedFileName
                            acceptAlternateNames:NO
                                            error:error];
    if (result) {
        return result;
    }
    // Fallback: accept index.ios.bundle (same as hot-updater) then any .jsbundle/.bundle
    return [self findMainBundleInFolder:folderPath
                     expectedFileName:expectedFileName
                acceptAlternateNames:YES
                                error:error];
}

+ (NSString *)findMainBundleInFolder:(NSString *)folderPath
                    expectedFileName:(NSString *)expectedFileName
               acceptAlternateNames:(BOOL)acceptAlternateNames
                               error:(NSError **)error
{
    NSArray* folderFiles = [[NSFileManager defaultManager]
                            contentsOfDirectoryAtPath:folderPath
    ...
    for (NSString *fileName in folderFiles) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:fullFilePath
            ...
            NSString *mainBundlePathInFolder = [self findMainBundleInFolder:fullFilePath
                                                           expectedFileName:expectedFileName
                                                      acceptAlternateNames:acceptAlternateNames
                                                                      error:error];
            if (mainBundlePathInFolder) {
                return [fileName stringByAppendingPathComponent:mainBundlePathInFolder];
            }
        } else {
            BOOL isMatch = [fileName isEqualToString:expectedFileName];
            if (!isMatch && acceptAlternateNames) {
                isMatch = [fileName isEqualToString:@"index.ios.bundle"]
                    || [[fileName pathExtension] isEqualToString:@"jsbundle"]
                    || [[fileName pathExtension] isEqualToString:@"bundle"];
            }
            if (isMatch) {
                return fileName;
            }
