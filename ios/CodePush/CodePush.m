 ...
 * Used for asset resolution and getBaseURL() parity with Android / hot-updater.
 * Returns nil if no CodePush update is currently installed (e.g. running from binary).
 */
RCT_EXPORT_METHOD(getPackageFolderPath:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    NSError *error = nil;
    NSString *packageFolder = [CodePushPackage getCurrentPackageFolderPath:&error];
    if (packageFolder && packageFolder.length > 0) {
        NSURL *fileURL = [NSURL fileURLWithPath:packageFolder isDirectory:YES];
        NSString *baseURL = fileURL.absoluteString;
        CPLog(@"getPackageFolderPath: %@", baseURL);
        resolve(baseURL);
    } else {
        CPLog(@"getPackageFolderPath: No CodePush update installed");
        resolve(nil);
    }
}

RCT_EXPORT_METHOD(getCurrentPackageBundlePath:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    NSError *error = nil;
    NSString *bundlePath = [CodePushPackage getCurrentPackageBundlePath:&error];
...
/*
 * Debug: returns the current bundle path and folder listing for the CodePush package.
 * Resolves with a string: "BUNDLE_PATH=<file://...>\n\nFOLDER:\n<item1>\n<item2>..." or nil if no update.
 */
RCT_EXPORT_METHOD(getBundlePathAndFolderListing:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    NSError *error = nil;
    NSString *packageFolder = [CodePushPackage getCurrentPackageFolderPath:&error];
    ...
    NSString *bundlePath = [CodePushPackage getCurrentPackageBundlePath:&error];
    NSArray<NSString *> *contents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:packageFolder error:&error];
    NSURL *bundleURL = bundlePath ? [NSURL fileURLWithPath:bundlePath] : nil;
    NSString *bundlePathStr = bundleURL.absoluteString ?: @"(none)";
    NSString *listing = contents.count ? [contents componentsJoinedByString:@"\n"] : @"(empty)";
    NSString *result = [NSString stringWithFormat:@"BUNDLE_PATH=\n%@\n\nFOLDER CONTENTS:\n%@", bundlePathStr, listing];
    CPLog(@"getBundlePathAndFolderListing: %@", bundlePathStr);
    resolve(result);
}
